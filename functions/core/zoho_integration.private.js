...
exports.getAvailableAgents = async(zoho_api_key) => {
    try {
        ...
        if(rol_de_guardias.hangup) return rol_de_guardias
        if(active_users.hangup) return active_users

        let agents = active_users.data.map(x => {
            let data_from_rol_de_guardia = rol_de_guardias.data.find(y => x.id === y.Owner.id)
            let newData = {
