...
exports.handler = async function (context, event, callback) {
    ...
    try {
        ...
        const agents = await getAvailableAgents(zoho_api_key.data)
        const thread_id = await createNewThread(event, context.OPENAI_API_KEY, agents); 
        
        Cache.set(_THREAD_ID, thread_id.data)
        Cache.set(_ZOHO_API_KEY, zoho_api_key.data)
        ...
        twiml.say({ voice: context.AI_VOICE }, error.message);
        twiml.hangup();
